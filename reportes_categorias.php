...
<header class="rdm-toolbar--contenedor">
    <div class="rdm-toolbar--fila">
        <div class="rdm-toolbar--izquierda">
            <a href="reportes.php#categorias"><div class="rdm-toolbar--icono"><i class="zmdi zmdi-arrow-left zmdi-hc-2x"></i></div></a>
...
<main class="rdm--contenedor-toolbar">
    ...
    <section class="rdm-lista--porcentaje">
        ...
        <div class="rdm-tarjeta--primario-largo">
            <h1 class="rdm-tarjeta--titulo-largo">Categorias <?php echo ($rango); ?></h1>
        ...
        <?php
        //ventas por cada categoria
        $consulta = $conexion->query("SELECT count(categoria), categoria FROM ventas_productos WHERE local = '$sesion_local_id' and (estado = 'liquidado' or  estado = 'entregado') and fecha BETWEEN '$desde' and '$hasta' GROUP BY categoria ORDER BY count(categoria) DESC");                

        while ($fila = $consulta->fetch_assoc())
        {
            $categoria = $fila['categoria'];

            //consulto el total para cada categoria
            $consulta2 = $conexion->query("SELECT * FROM ventas_productos WHERE local = '$sesion_local_id' and (estado = 'liquidado' or  estado = 'entregado') and categoria = '$categoria' and fecha BETWEEN '$desde' and '$hasta'");
            $total_categoria = $consulta2->num_rows;
            ...
            $porcentaje_categoria = ($total_categoria / $total_productos) * 100;
            $porcentaje_categoria = number_format($porcentaje_categoria, 1, ".", ".");
            ...
            <article class="rdm-lista--item-porcentaje">
                <div>
                    <div class="rdm-lista--izquierda-porcentaje">
                        <h2 class="rdm-lista--titulo-porcentaje"><?php echo ucfirst($categoria); ?></h2>
                        <h2 class="rdm-lista--texto-secundario-porcentaje"><?php echo "$total_categoria"; ?> (<?php echo "$total_precio_final_t"; ?>)</h2>
                    </div>
                    <div class="rdm-lista--derecha-porcentaje">
                        <h2 class="rdm-lista--texto-secundario-porcentaje"><?php echo "$porcentaje_categoria"; ?>%</h2>
                    </div>
                </div>
                
                <div class="rdm-lista--linea-pocentaje-fondo" style="background-color: #B2DFDB">
                    <div class="rdm-lista--linea-pocentaje-relleno" style="width: <?php echo "$porcentaje_categoria"; ?>%; background-color: #009688;"></div>
                </div>
            </article>
